...
// local 변수 대비 반복 테케가 있는 경우(SWEA) 각 테케별로 초기화 해줘야 한다.
// 테케 입력을 파일로 처리
// 가장 큰 수와 다음 큰 수(가장 큰수와 동일할 수 있다.)
public class 큰수의법칙_6 {
	static int n, m, k, result; // static 은 자동으로 0으로 초기화 되기 때문에 선언 필요 x
	static int[] arr;
...
//        Arrays.sort(arr); // 입력 받은 수들 정렬하기 O(NlogN)
	...
	static int first() {
		int first = 0; // max
		int firstIndex = 0;
		for (int i = 0; i < n; i++) { // 가장 큰 수가 중복되어도 맨 앞의 수가 가장 큰 수가 처리되도록.
			if (arr[i] >= first) {
				first = arr[i];
				firstIndex = i;
			}
