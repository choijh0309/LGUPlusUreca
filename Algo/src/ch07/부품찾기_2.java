...
public class 부품찾기_2 {
    ...
    // 특정 배열에서 원하는 값이 index 를 return 없으면 -1 리턴 // 이진 탐색으로 처리
    static int binarySearch(int[] arr, int target, int start, int end) { 
    	while (start <= end) {
    		int mid = (start + end) / 2; // 중간값
    		if (arr[mid] == target) return mid; // 중간값과 찾는 값 일치 (찾았다.)
    		else if (arr[mid] > target) end = mid - 1; // 못 찾았다. 중간값이 찾는 값보다 더 크다. 왼쪽에서 찾아야 한다.
    		else start = mid + 1; // 못 찾았다. 중간값이 찾는 값보다 더 작다. 오른쪽에서 찾아야 한다.
    	}
    	return -1;
    }
